...
Though it may be hard to recognize, the right side is equal to a simple derivative (the chain rule is used):
...
For the integrating factor \textit{only}, we can ignore the typical $+ c$ constant. However, for the final solution we will often see equations in the following form:
