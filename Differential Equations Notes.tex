...
\begin{center}
	\begin{tikzpicture}[scale=1.625]
		...
		\def\eqtn{(y+1.1)*(y-1.1)*(y+1)*(y+1)}
		\begin{axis}[
			axis x line=center,
			axis y line=center,
			xtick={\xmin,\xmin+0.1,...,\xmax},
			xticklabels={},
			extra x ticks={\xmin,-1,1,\xmax},
			ytick={\ymin,\ymin+0.1,...,\ymax},
			yticklabels={},
			extra y ticks={\ymin,-1,1,\ymax},
			xlabel={$x$},
			ylabel={$y$},
			xlabel style={right},
			ylabel style={above},
			...
			%\addplot[color=magenta,samples=100]{-x - 1 - 3*e^x};
			\foreach \n in {\ymin,-1.8,...,\ymax}
			\addplot[blue,quiver={u={1/sqrt(1+(\eqtn)^2)},v={(\eqtn)/sqrt(1+(\eqtn)^2)},scale arrows=0.25},->,samples=20,]{\n};
		\end{axis}
	\end{tikzpicture}
\end{center}
