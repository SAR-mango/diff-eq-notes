...
\textit{For the integrating factor \textbf{only}, we can ignore the typical $+\; c$ constant}. However, when solving for $y(t)$ we will often need to exponentiate functions of the form $a(t) + c$, where $c$ is a constant. The constant falls out of the exponential and instead goes beside it. Here's what that looks like:
$$e^{a(t) + c} = e^{a(t)}e^{c} = ce^{a(t)}$$

In other words, the constant falls out of the exponential and instead goes beside it.
...
%			ymin=\ymin-0.25,
%			ymax=\ymax+0.25]
%			\addplot[color=magenta,samples=100]{2+19^(x-2.8)};
%			\addplot[color=magenta,samples=100]{2-19^(x-2.8)};
%			\foreach \n in {\ymin,-2.75,...,\ymax}
