...
For the integrating factor \textit{only}, we can ignore the typical $+\; c$ constant. However, when solving for $y(t)$ we will often need to exponentiate functions of the form $a(t) + c$, where $c$ is a constant. The constant falls out of the exponential and instead goes beside it. Here's what that looks like:
