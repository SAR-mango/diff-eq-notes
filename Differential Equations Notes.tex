...
\textit{Solution:}
\par Critical points at $y = -3$, $y = 0$, and $y = 2$.
\par $y = -3$ is unstable, $y = 0$ is stable, and $y = 2$ is semi-stable.
\par Note that a differential equation need not contain equilibrium solutions of all kinds of stability, and it need not contain equilibrium solutions at all.
