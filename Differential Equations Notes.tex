...
\begin{enumerate}[wide=20pt]
	\item[$\bold{y = 2}$:] The slope above $y = 2$ is always positive, so if we are pushed upwards from $y = 2$ we will go flying up and away, never to return.
	
\end{enumerate}

The slope above $y = 2$ is always positive, so if we are pushed upwards from $y = 2$ we will go flying up and away, never to return.

Similarly, the slope below $y = 2$ is always negative, so if we are pushed slightly downwards off of $y = 2$ we will go down – again, never to return.
