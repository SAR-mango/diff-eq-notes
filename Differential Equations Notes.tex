...
\begin{center}
	\begin{tikzpicture}
		...
		\begin{axis}[
			...
			ymin=\ymin-0.5,
			ymax=\ymax+0.5]
			\node[label={270:{$-2$}},circle,draw=magenta,line width=1mm,inner sep=0pt,minimum size=1mm] at(-2,0){};
			\node[label={270:{$2$}},circle,draw=magenta,line width=1mm,inner sep=0pt,minimum size=1mm] at(2,0){};
			\node[label={270:{$-1$}},circle,draw=magenta,line width=1mm,inner sep=0pt,minimum size=1mm] at(-1,0){};
...
What does the phase line tell us? Notice the magenta points – these are the equilibrium solutions of our differential equation. Then look at the arrows beside each point. They simply tell you which direction you would move if you were nudged towards the arrow.
\end{document}